...
const SIGUIENTES: Record<string, { estado: string; texto: string; clase: string }[]> = {
  ...
  PRESENTADO: [
    ...
    { estado: 'RECHAZADO', texto: 'Rechazado', clase: 'border border-slate-300 bg-white text-slate-700 hover:bg-slate-50' },
  ],
  APROBADO: [
      ...
      <article className="tarjeta mx-auto max-w-4xl p-10">
        <header className="mb-6 flex items-start justify-between gap-6 border-b-2 border-slate-800 pb-4">
          ...
          <div className="text-right">
            <p className="text-xs uppercase tracking-widest text-slate-400">Informe de prestaciones</p>
            <p className="text-2xl font-bold text-slate-900">Nº {informe.folio}</p>
            <p className="text-sm text-slate-600">{fechaLarga(informe.fechaEmision)}</p>
          </div>
        </header>
        ...
        <section className="mb-6 grid gap-x-8 gap-y-2 rounded-lg bg-slate-50 p-4 sm:grid-cols-2">
        ...
        <h2 className="mb-2 text-sm font-bold uppercase tracking-widest text-slate-500">Prestaciones</h2>
        <ContenedorTabla>
          <thead>
            <tr>
              <th>Fecha</th>
          ...
          <tbody>
            {informe.items.map((item) => (
              <tr key={item.id}>
                <td className="whitespace-nowrap text-slate-600">{fechaCorta(item.fecha)}</td>
                <td className="font-mono text-xs text-slate-600">{item.codigoPrestacion ?? '—'}</td>
                <td className="text-slate-800">{item.descripcion}</td>
                <td className="text-xs text-slate-600">{item.profesional ?? '—'}</td>
        ...
        <div className="mt-6 flex justify-end">
          <dl className="w-full max-w-sm space-y-1.5 text-sm">
            <div className="flex justify-between">
              <dt className="text-slate-600">Total prestaciones</dt>
              <dd className="tabular-nums text-slate-800">{clp(informe.totalPrestaciones)}</dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-slate-600">Cubierto por el convenio</dt>
        ...
        {informe.observaciones && (
          <section className="mt-6 rounded-lg bg-slate-50 p-4">
            ...
            <p className="mt-1 whitespace-pre-wrap text-sm text-slate-700">{informe.observaciones}</p>
          </section>
        )}
            ...
            <div className="border-t border-slate-800 pt-1">
              {informe.profesional ? (
                <>
                  ...
                  <p className="text-xs text-slate-600">{informe.profesional.especialidad}</p>
                  {informe.profesional.registroSuperintendencia && (
                    <p className="text-xs text-slate-600">Reg. Nº {informe.profesional.registroSuperintendencia}</p>
                  )}
                </>
              ) : (
                <p className="text-sm font-semibold text-slate-900">{config?.nombreClinica ?? 'MEDIGEX'}</p>
              )}
            </div>
          </div>
        </footer>
